...
public class L2Spawn
{
	...
	private L2Npc initializeNpcInstance(L2Npc mob)
	{
		...
		{
			((L2Attackable) mob).setEasyChampion(false);
		}
		
		if (Config.EASY_CHAMPION_ENABLE)
		{
			// Set champion on next spawn
			if ((mob instanceof L2MonsterInstance) && !getTemplate().isQuestMonster() && !mob.isRaid() && !((L2MonsterInstance) mob).isRaidMinion() && (Config.EASY_CHAMPION_FREQUENCY > 0) && (mob.getLevel() >= Config.EASY_CHAMP_MIN_LVL) && (mob.getLevel() <= Config.EASY_CHAMP_MAX_LVL) && (Config.EASY_CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)))
			{
				int random = Rnd.get(100);
				
				if (random < Config.EASY_CHAMPION_FREQUENCY)
				{
					((L2Attackable) mob).setEasyChampion(true);
				}
			}
		}
		...
		if (Config.HARD_CHAMPION_ENABLE)
		{
			// Set champion on next spawn
			if ((mob instanceof L2MonsterInstance) && !getTemplate().isQuestMonster() && !mob.isRaid() && !((L2MonsterInstance) mob).isRaidMinion() && (Config.HARD_CHAMPION_FREQUENCY > 0) && (mob.getLevel() >= Config.HARD_CHAMP_MIN_LVL) && (mob.getLevel() <= Config.HARD_CHAMP_MAX_LVL) && (Config.HARD_CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)))
			{
				int random = Rnd.get(100);
				
				if (random < Config.HARD_CHAMPION_FREQUENCY)
				{
