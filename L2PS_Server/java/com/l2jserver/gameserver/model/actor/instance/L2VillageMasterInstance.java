	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			else if (player.isInCombat())
			{
				...
				_log.info("Player " + player.getName() + " trying to cheating with Subclass. But this bug is not working on L2PS");
				return;
			}
			...
			{
				player.sendMessage("Sub classes may not be created or changed while you are not in peaceful zone.");
				_log.info("Player " + player.getName() + " trying to cheating with Subclass. But this bug is not working on L2PS");
				return;
			}
