...
public class L2Attackable extends L2Npc
{
	...
	private boolean _Easychampion = false;
	private boolean _Hardchampion = false;
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer instanceof L2PcInstance)
		{
			...
			{
				((L2PcInstance) killer).setKills(((L2PcInstance) killer).getKills() + Config.START_CAPTCHA);
				if ((((L2PcInstance) killer).getKills() >= Config.GET_KILLS) || (((L2PcInstance) killer).getKills() == 0))
				{
				...
				for (RewardInfo reward : rewards.values())
				{
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (Config.EASY_CHAMPION_ENABLE && isEasyChampion())
							{
								exp *= Config.EASY_CHAMPION_REWARDS;
								sp *= Config.EASY_CHAMPION_REWARDS;
							}
							else if (Config.HARD_CHAMPION_ENABLE && isHardChampion())
							{
								exp *= Config.HARD_CHAMPION_REWARDS;
								sp *= Config.HARD_CHAMPION_REWARDS;
							}
	...
	private ItemHolder calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		// Applies Drop rates
		if ((Config.PREMIUM_RATE_DROP_ITEMS_ID.containsKey(drop.getItemId())) && (lastAttacker.getPremiumService() == 1))
		{
		...
		if (Config.EASY_CHAMPION_ENABLE && isEasyChampion())
		{
			dropChance *= Config.EASY_CHAMPION_REWARDS;
		}
		else if (Config.HARD_CHAMPION_ENABLE && isHardChampion())
		{
			dropChance *= Config.HARD_CHAMPION_REWARDS;
		}
		...
		if (Config.EASY_CHAMPION_ENABLE && isEasyChampion() && ((drop.getItemId() == PcInventory.ADENA_ID) || Util.contains(SevenSigns.SEAL_STONE_IDS, drop.getItemId())))
		{
			itemCount *= Config.EASY_CHAMPION_ADENAS_REWARDS;
		}
		else if (Config.HARD_CHAMPION_ENABLE && isHardChampion() && ((drop.getItemId() == PcInventory.ADENA_ID) || Util.contains(SevenSigns.SEAL_STONE_IDS, drop.getItemId())))
		{
			itemCount *= Config.HARD_CHAMPION_ADENAS_REWARDS;
		}
	...
	private ItemHolder calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (Config.EASY_CHAMPION_ENABLE && isEasyChampion())
		{
			categoryDropChance *= Config.EASY_CHAMPION_REWARDS;
		}
		else if (Config.HARD_CHAMPION_ENABLE && isHardChampion())
		{
			categoryDropChance *= Config.HARD_CHAMPION_REWARDS;
		}
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		if (Config.EASY_CHAMPION_ENABLE && isEasyChampion() && ((Config.EASY_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0) || (Config.EASY_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE > 0)))
		{
			int champqty = Rnd.get(Config.EASY_CHAMPION_REWARD_QTY);
			ItemHolder item = new ItemHolder(Config.EASY_CHAMPION_REWARD_ID, ++champqty);
			
			if ((player.getLevel() <= getLevel()) && (Rnd.get(100) < Config.EASY_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE))
			{
				if (Config.AUTO_LOOT || isFlying())
				{
					player.addItem("EasyChampionLoot", item.getId(), item.getCount(), this, true); // Give the item(s) to the L2PcInstance that has killed the L2Attackable
				}
				else
				{
			...
			else if ((player.getLevel() > getLevel()) && (Rnd.get(100) < Config.EASY_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE))
			{
				if (Config.AUTO_LOOT || isFlying())
				{
					player.addItem("EasyChampionLoot", item.getId(), item.getCount(), this, true); // Give the item(s) to the L2PcInstance that has killed the L2Attackable
		...
		// Apply Special Item drop with random(rnd) quantity(qty) for hard champions.
		else if (Config.HARD_CHAMPION_ENABLE && isHardChampion() && ((Config.HARD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0) || (Config.HARD_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE > 0)))
		{
			int hardchampqty = Rnd.get(Config.HARD_CHAMPION_REWARD_QTY);
			ItemHolder item = new ItemHolder(Config.HARD_CHAMPION_REWARD_ID, ++hardchampqty);
			
			if ((player.getLevel() <= getLevel()) && (Rnd.get(100) < Config.HARD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE))
			{
				if (Config.AUTO_LOOT || isFlying())
				{
					player.addItem("HardChampionLoot", item.getId(), item.getCount(), this, true); // Give the item(s) to the L2PcInstance that has killed the L2Attackable
				}
				else
				{
					dropItem(player, item);
			...
			else if ((player.getLevel() > getLevel()) && (Rnd.get(100) < Config.HARD_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE))
			{
				if (Config.AUTO_LOOT || isFlying())
				{
					player.addItem("HardChampionLoot", item.getId(), item.getCount(), this, true); // Give the item(s) to the L2PcInstance that has killed the L2Attackable
	...
	{
		if (isEasyChampion() && !Config.EASY_CHAMPION_ENABLE_VITALITY)
		{
			return false;
		}
		else if (isHardChampion() && !Config.HARD_CHAMPION_ENABLE_VITALITY)
		{
			return false;
		}
	...
	public void setEasyChampion(boolean Easychamp)
	{
		_Easychampion = Easychamp;
	}
	
	@Override
	public boolean isEasyChampion()
	{
		return _Easychampion;
	}
	
	public void setHardChampion(boolean Hardchamp)
	{
		_Hardchampion = Hardchamp;
	}
	
	@Override
	public boolean isHardChampion()
	{
		return _Hardchampion;
	}
