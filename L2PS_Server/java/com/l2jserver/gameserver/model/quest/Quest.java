...
public class Quest extends ManagedScript
{
	...
	public static boolean giveItemRandomly(L2PcInstance player, L2Npc npc, int itemId, long minAmount, long maxAmount, long limit, double dropChance, boolean playSound)
	{
		...
		if ((npc != null) && Config.EASY_CHAMPION_ENABLE && npc.isEasyChampion())
		{
			dropChance *= Config.EASY_CHAMPION_REWARDS;
			if ((itemId == PcInventory.ADENA_ID) || (itemId == PcInventory.ANCIENT_ADENA_ID))
			{
				minAmount *= Config.EASY_CHAMPION_ADENAS_REWARDS;
				maxAmount *= Config.EASY_CHAMPION_ADENAS_REWARDS;
			}
			else
			{
				minAmount *= Config.EASY_CHAMPION_REWARDS;
				maxAmount *= Config.EASY_CHAMPION_REWARDS;
			}
		}
