...
public class AutoRaidEventManager
{
	...
	public void onRaidDeath(MonsterInstance raid)
	{
		...
		Announcements.getInstance().announceToAll(_raid.getName() + " has fallen in the hands of powerful warriors. He will be back tomorrow!");
	...
	protected class RaidSpawnTask implements Runnable
	{
		...
		public void run()
		{
			...
			Announcements.getInstance().announceToAll(_raid.getName() + " has spawned in " + _currentLocation.getName() + " and will be there for the next 1 hour!");
	...
	protected class RaidNotifyTask implements Runnable
	{
		...
		public void run()
		{
			...
			Announcements.getInstance().announceToAll(_raid.getName() + " is in " + _currentLocation.getName() + ", so come and kill it now!");
		}
	}
	...
	protected class RaidDespawnTask implements Runnable
	{
		...
		public void run()
		{
			...
			if (_raid != null)
			{
				...
				Announcements.getInstance().announceToAll(_raid.getName() + " wasn't killed in time and escaped. He will be back tomorrow!");
