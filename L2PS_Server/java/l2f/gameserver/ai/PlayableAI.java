...
import static l2f.gameserver.ai.CtrlIntention.*;
...
public class PlayableAI extends CharacterAI
{
	...
	public void lockTarget(Creature target)
	{
		...
		else if (actor.getAggressionTarget() == null)
		{
			...
			/*
			 * if (getIntention() == CtrlIntention.AI_INTENTION_ATTACK) setAttackTarget(target); switch(getIntention()) { case AI_INTENTION_ATTACK: setAttackTarget(target); break; case AI_INTENTION_CAST: L2Skill skill = actor.getCastingSkill(); if(skill == null) skill = _skill; if(skill != null &&
			 * !skill.isUsingWhileCasting()) switch(skill.getTargetType()) { case TARGET_ONE: case TARGET_AREA: case TARGET_MULTIFACE: case TARGET_TUNNEL: setAttackTarget(target); actor.setCastingTarget(target); break; } break; }
			 */
