	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			case admin_add_mail:
				...
				{
					for(int num = 1; num < wordList.length; num++)
					{
						targetToAdd = wordList[num];
						if(!nicks.contains(targetToAdd))
						{
							if(CharacterDAO.getInstance().getObjectIdByName(targetToAdd) != 0)
							{
								nicks.add(targetToAdd);
								activeChar.sendMessage("Player " + targetToAdd + " was added to the list!");
							}
							else
								activeChar.sendMessage("Player " + targetToAdd + " not found!");
						}
					}
				}
				...
				{
					activeChar.sendMessage("Target a player and use //add_mail or use //add_mail nick nick nick nick");
					return false;
				}
