...
/**
 * Para manejar un html dinamico que muestra todas las augmentaciones disponibles para armas
 * Permite asignarle cualquier augmentation que se elija al arma del target
 *
 * @author Synerge
 */
public class AdminAugmentation implements IAdminCommandHandler
{
	...
	private static final int MAX_AUGMENTATIONS_PER_PAGE = 10;
	...
	protected void showMainMenu(Player player)
	{
		...
		for (OptionDataTemplate augm : augmentations)
		{
			...
			augmentationsHtm.append("<table " + (lastColor ? "bgcolor=131210" : "") + ">");
			augmentationsHtm.append("<tr>");
			augmentationsHtm.append("<td width=32 background=" + (skill != null ? skill.getIcon() : "icon.skill5041") + "><button action=\"bypass -h admin_augmentation setaug " + augm.getId() + "\" width=32 height=32 back=\"\" fore=\"\"></td>");
