...
/**
 * Class that is handling Bot Reports of the player
 * Checking if Bot can be reported, handling Captcha Window
 * Handling Captcha Answer and Success/Fail of it
 */
public class CaptchaHandler
{
	...
	/**
	 * Checking if Actor can Report Player.<p/>
	 * Sending Message to the Actor if Target cannot be Reported<p/>
	 * Return false on:
	 * <ul>
	...
	private static boolean canReport(Player actor, Player target)
	{
		...
		if(actor.getIP().equals(target.getIP()) || actor.getHWID().equals(target.getHWID()))
		{
	...
	/**
	 * Trying to Report Target
	 * Checking if actor can report target, sending message to actor if report was OK
	 * If actor could report target:
	 * - Adding quickVar LastCaptchaTest to the target
	 * - Adding Event to CaptchaTimers list
	 * - Generating and Sending Captcha Window to target
	 * @param actor Player that wants to report target
	...
	/**
	 * Handling Captcha Answer
	 * If Captcha Event is null, sending message to actor
	 * If Captcha is correct: {@link #onCorrectCaptcha(CaptchaEvent)}
	 * If Captcha is wrong: {@link #onFailedCaptcha(CaptchaEvent)}
	...
	/**
	 * Removing Event from Captcha Event List
	 * Jailing Player that failed Captcha, sending him message if he is still online
	 * If reporter is online, sending him message and giving back one Report Try
	 * @param event Captcha Event of the Target TimeStamp
	 */
	...
	private static void punishment(String target)
	{
		...
		String data = Config.CAPTCHA_PUNISHMENT[count >= Config.CAPTCHA_PUNISHMENT.length ? Config.CAPTCHA_PUNISHMENT.length - 1 : count];
		...
		if(type.equals("JAIL"))
		{
			AutoBan.doJailPlayer(target, (size * 60) * TimeUtils.SECOND_IN_MILLIS, true);
	...
	/**
	 * Removing Event from Captcha Event List
	 * Sending Messages to both, reporter and target.
	 * Both can be null
