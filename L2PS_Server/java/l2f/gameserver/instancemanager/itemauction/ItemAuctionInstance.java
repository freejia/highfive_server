...
public class ItemAuctionInstance
{
		...
		switch(auctions.length)
		{
			...
			default:
			{
				Arrays.sort(auctions, new Comparator<ItemAuction>(){
					...
					{
						if(o2.getStartingTime() > o1.getStartingTime()){ return 1; }
						if(o2.getStartingTime() < o1.getStartingTime()){ return -1; }
	...
	private class ScheduleAuctionTask extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			switch(state)
			{
				case CREATED:
				{
					if(!_auction.setAuctionState(state, ItemAuctionState.STARTED)){ throw new IllegalStateException("Could not set auction state: " + ItemAuctionState.STARTED.toString() + ", expected: " + state.toString()); }

					_log.info("ItemAuction: Auction " + _auction.getAuctionId() + " has started for instance " + _auction.getInstanceId());
					if(Config.ALT_ITEM_AUCTION_ENABLED && Config.ALT_ITEM_AUCTION_START_ANNOUNCE)
					{
						String[] params = {};
				...
				case STARTED:
				{
					...
					if(!_auction.setAuctionState(state, ItemAuctionState.FINISHED)){ throw new IllegalStateException("Could not set auction state: " + ItemAuctionState.FINISHED.toString() + ", expected: " + state.toString()); }
	...
	void onAuctionFinished(ItemAuction auction)
	{
		...
		if(Config.ALT_ITEM_AUCTION_ENABLED)
		{
			...
			ItemAuctionInstance next = ItemAuctionManager.getInstance().getManagerInstance(auction.getInstanceId());
			ItemAuction _auction = next != null ? next.getNextAuction() : null;
			String time = _auction != null ? DATE.format(new Date(_auction.getStartingTime())) : "Unknown!";

			String text = "Item auction finished in " + getName(auction.getInstanceId()) + ". The next auction will begin on the: " + time;

			for(Player player : GameObjectsStorage.getAllPlayersForIterate())
			{
				player.sendPacket(new Say2(0, ChatType.ANNOUNCEMENT, "", text));
			}
		}
	}

	private String getName(int auctionId)
	{
		switch(auctionId)
		{
			case 32320:
