...
public class Log
{
	...
	private static void LogActionItem(String action, String activeChar, String process, String item, long count)
	{
		...
		if (_logItems == null || c.get(Calendar.DAY_OF_MONTH) != DAY)
		{
			if (_logItems != null)
				closeLogger();
	...
	/**
	 * Logging Short Message to Items Log. Time will be included
	...
	{
		Calendar c = Calendar.getInstance();
		return fileType+ ' ' +c.get(Calendar.DAY_OF_MONTH)+ '-' +(c.get(Calendar.MONTH)+1)+ '-' +c.get(Calendar.YEAR);
	}
