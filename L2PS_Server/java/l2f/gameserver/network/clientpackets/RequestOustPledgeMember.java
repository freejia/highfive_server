...
public class RequestOustPledgeMember extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		for(final Skill skill : clan.getSkills())
		{
			memberPlayer.removeSkill(skill);
		}

		final SubUnit subUnit = clan.getSubUnit(subUnitType);
		...
		memberPlayer.sendPacket(new SkillList(memberPlayer));

		if(!memberPlayer.isNoble())
			memberPlayer.setTitle("");

		memberPlayer.broadcastUserInfo(true);
