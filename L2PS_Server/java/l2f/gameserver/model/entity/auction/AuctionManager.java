...
public class AuctionManager
{
	...
	public void buyItem(Player buyer, ItemInstance item, long quantity)
	{
		...
		buyer.getInventory().reduceAdena(auction.getPricePerItem()*quantity, "Auction Online Bought");
		...
		if (seller != null)
		{
			...
			seller.addAdena(auction.getPricePerItem()*quantity, true, "Auction Online Sold");
		}
		else
		{
			addAdenaToSeller(auction.getSellerObjectId(), auction.getPricePerItem()*quantity);
			Log.LogAddItem(auction.getSellerName() + '[' + auction.getSellerObjectId() + ']', "Auction Offline", "Adena", auction.getPricePerItem() * quantity);
		}
	...
	private AuctionItemTypes getItemGroup(ItemInstance item)
	{
		if (item.isEquipable())
		{
			...
			if (item.getBodyPart() == ItemTemplate.SLOT_HAIR || item.getBodyPart() == ItemTemplate.SLOT_HAIRALL
					|| item.getBodyPart() == ItemTemplate.SLOT_DHAIR)
