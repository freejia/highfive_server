...
public class Olympiad
{
	...
	public static void init()
	{
		...
		_scheduledOlympiadEnd = ThreadPoolManager.getInstance().schedule(new OlympiadEndTask(), getMillisToCachedOlyEnd());

		updateCompStatus();

		if(_scheduledWeeklyTask != null)
			_scheduledWeeklyTask.cancel(false);
		_scheduledWeeklyTask = ThreadPoolManager.getInstance().schedule(new WeeklyTask(), getMillisToWeekChange());
	}
	...
	public static synchronized boolean registerNoble(Player noble, CompType type)
	{
		...
		if(getMillisToCachedOlyEnd() <= 600 * 1000)
		{
			noble.sendPacket(SystemMsg.THE_GRAND_OLYMPIAD_GAMES_ARE_NOT_CURRENTLY_IN_PROGRESS);
			return false;
		}
		...
		if(!validPlayer(noble, noble, type)){ return false; }

		if(getNoblePoints(noble.getObjectId()) < Config.OLY_POINT_MIN_LIMIT)
		{
			noble.sendMessage(new CustomMessage("l2f.gameserver.model.entity.Olympiad.LessPoints", noble).addNumber(Config.OLY_POINT_MIN_LIMIT));
			return false;
		}
	...
	public static synchronized void logoutPlayer(Player player)
	{
		...
		_playersIp.remove(player.getHWID());
	...
	protected static long getMillisToWeekChange()
	{
		return TimeUtils.getMilisecondsToNextDay(Config.ALT_OLY_WPERIOD, 3);
	}

	private static long getMillisToCachedOlyEnd()
	{
		return _olympiadEnd;
	}
