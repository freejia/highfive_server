...
public class CCPPasswordRecover
{
	...
	public static void setupFirstStage(Player player, String[] args)
	{
		...
		//Sending mail
		sendEmail(args[3], "Hello!<br>To complete your Password Recovery setup in L2Conflict, use the code inside TextBox in game!<br>Code: "+generateCode(player)+"<br>Thank You!");
	...
	public static void resetSecondPage(Player player, String account, String answer, String page)
	{
		...
		//Sending mail
		sendEmail(emailAns[0], "Hello!<br>"+actionDesc+" in L2Conflict, use the code inside TextBox in game!<br>Code: "+generateCode(player)+"<br>Thank You!");
	...
	private static long generateCode(Player player)
	{
		...
		Long[] array = {System.currentTimeMillis(), toLong};
	...
	private static void sendEmail(final String email, final String message)
	{
		ThreadPoolManager.getInstance().execute(new Runnable()
		{
			...
			public void run()
			{
				try
				{
					finalSendMail("l2talesrecovery", "TAleS2283631", email, "", "L2Tales Code!", message);
				}
	...
	private static String getSecurityQuestion(String accountName)
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT securityQuestion FROM accounts WHERE login='"+accountName+"'");
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT securityAnswer, email FROM accounts WHERE login='"+accountName+"'");
				ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
	...
	public static boolean isRecoveryCompleted(String accountName)
	{
		boolean contains = false;
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("SELECT email FROM accounts WHERE login='"+accountName+"'");
				ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE accounts SET securityQuestion=?,securityAnswer=?,email=? WHERE login=?"))
		{
