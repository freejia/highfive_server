...
public class CastleSiegeEvent extends SiegeEvent<Castle, SiegeClanObject>
{
	...
	{
		if(getResidence().getSiegeDate().getTimeInMillis() != 0){ return; }
	...
	{
		if(!_nextSiegeTimes.contains(id) || (_nextSiegeDateSetTask == null)){ return; }
	...
	public boolean canRessurect(Player resurrectPlayer, Creature target, boolean force)
	{
		...
		if(!playerInZone && !targetInZone){ return true; }
		// если таргет вне осадный зоны - рес разрешен
		if(!targetInZone){ return false; }
		...
		else
		{
			...
			// Synerge - If two or more of the towers have been destroyed: Neither the resurrection spell nor the scroll may be used
			if(deadTowers >= 2)
			{
