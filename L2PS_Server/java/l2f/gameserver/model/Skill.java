...
public abstract class Skill extends StatTemplate implements Cloneable
{
	...
	public static enum SkillType
	{
		...
		EFFECT(l2f.gameserver.skills.skillclasses.Effect.class),
		...
		HARDCODED(l2f.gameserver.skills.skillclasses.Effect.class),
		...
		SUMMON(l2f.gameserver.skills.skillclasses.Summon.class),
	...
	public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if ((activeChar.isPlayer()) && (this._vitConsume > 0))
		{
			Player p = (Player) activeChar;
			if (p.getVitality() < (this._vitConsume + 1))
			{
				p.sendPacket(Msg.NOT_ENOUGH_MATERIALS);
				return false;
			}
		}
		else if ((!activeChar.isPlayer()) && (this._vitConsume > 0))
		{
			return false;
		}
		...
		if (activeChar.isFlyingDisabled() && (_flyType == FlyType.DUMMY || _flyType == FlyType.CHARGE))
		{
	...
	public SystemMsg checkTarget(Creature activeChar, Creature target, Creature aimingTarget, boolean forceUse, boolean first)
	{
		...
		if (player != null)
		{
			...
			if (pcTarget != null)
			{
				...
				if ((player.isInTvT()) && (player.getTeam() != TeamType.NONE) && (pcTarget.getTeam() == TeamType.NONE))
					return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				if ((player.isInTvT()) && (pcTarget.getTeam() != TeamType.NONE) && (player.getTeam() == TeamType.NONE))
					return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				...
				if (isOffensive())
				{
					...
					if ((player.isInTvT()) && (player.getTeam() != TeamType.NONE) && (pcTarget.getTeam() != TeamType.NONE) && (player.getTeam() == pcTarget.getTeam()) && (player != pcTarget) && (!(player.isInLastHero())))
						return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				...
				if ((player.isInTvT()) && (player.getTeam() != TeamType.NONE) && (pcTarget.getTeam() != TeamType.NONE) && (player.getTeam() != pcTarget.getTeam()))
					return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
				...
				for (EffectTemplate et : getEffectTemplates())
				{
					...
					for (Creature target : targets)
					{
						...
						if (e != null)
						{
							...
							if (e.isOneTime())
							{
								// Ð­Ñ„Ñ„ÐµÐºÑ‚Ñ‹ Ð¾Ð´Ð½Ð¾ÐºÑ€Ð°Ñ‚Ð½Ð¾Ð³Ð¾ Ð´ÐµÐ¹Ñ�Ñ‚Ð²Ð¸Ñ� Ð½Ðµ ÑˆÐµÐ´ÑƒÐ»Ñ�Ñ‚Ñ�Ñ�, Ð° Ð¿Ñ€Ð¸Ð¼ÐµÐ½Ñ�ÑŽÑ‚Ñ�Ñ� Ð½ÐµÐ¼ÐµÐ´Ð»ÐµÐ½Ð½Ð¾
								// ÐšÐ°Ðº Ð¿Ñ€Ð°Ð²Ð¸Ð»Ð¾ Ñ�Ñ‚Ð¾ Ð¿Ð¾Ð±Ð¾Ñ‡Ð½Ñ‹Ðµ Ñ�Ñ„Ñ„ÐµÐºÑ‚Ñ‹ Ð´Ð»Ñ� Ñ�ÐºÐ¸Ð»Ð»Ð¾Ð² Ð¼Ð¾Ð¼ÐµÐ½Ñ‚Ð°Ð»ÑŒÐ½Ð¾Ð³Ð¾ Ð´ÐµÐ¹Ñ�Ñ‚Ð²Ð¸Ñ�
								if (e.checkCondition())
								{
									e.onStart();
									e.onActionTime();
	...
	/**
	 * @return Ð’Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°ÐµÑ‚ Ð¸Ð´ Ð¿Ñ€ÐµÐ´Ð¼ÐµÑ‚Ð°(Ñ‚Ð°Ð»Ð¸Ñ�Ð¼Ð°Ð½Ð°) Ð¼Ð°Ð½Ñƒ ÐºÐ¾Ñ‚Ð¾Ñ€Ð¾Ð³Ð¾ Ð½Ð°Ð´Ð¾ Ð¸Ñ�Ð¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ñ‚ÑŒ
	 */
	...
	/**
	 * @return Ð’Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°ÐµÑ‚ Ð¸Ñ�Ð¿Ð¾Ð»ÑŒÐ·ÑƒÐµÐ¼Ð¾Ðµ Ð´Ð»Ñ� ÐºÐ°Ñ�Ñ‚Ð° ÐºÐ¾Ð»Ð¸Ñ‡ÐµÑ�Ñ‚Ð²Ð¾ Ð¼Ð°Ð½Ñ‹ Ð¿Ñ€ÐµÐ´Ð¼ÐµÑ‚Ð°(Ñ‚Ð°Ð»Ð¸Ñ�Ð¼Ð°Ð½Ð°)
	 */
