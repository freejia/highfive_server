...
public class Party implements PlayerGroup
{
	...
	public Party(Player leader, int itemDistribution)
	{
		...
		// Synerge - Add this party to the static manager
		getParties().put(_partyId, this);
	}
	...
	public void dissolveParty()
	{
		...
		// Synerge - Remove this party from the static manager
		getParties().remove(_partyId);
	}
	...
	private void distributeItem0(Player player, ItemInstance item, NpcInstance fromNpc)
	{
		...
		switch (_itemDistribution)
		{
			...
			case ITEM_RANDOM_SPOIL:
				...
				for (Player member : _members)
				{
					if (member.isInRangeZ(player, Config.ALT_PARTY_DISTRIBUTION_RANGE) && member.getInventory().validateCapacity(item) && member.getInventory().validateWeight(item))
					{
						ret.add(member);
					}
				}
			...
			case ITEM_ORDER_SPOIL:
				synchronized (_members)
				{
					...
					while ((target == null) && !ret.isEmpty())
					{
						...
						{
							if (looterPlayer.isInRangeZ(player, Config.ALT_PARTY_DISTRIBUTION_RANGE) && ItemFunctions.canAddItem(looterPlayer, item))
							{
								target = looterPlayer;
							}
							else
	...
	private void distributeAdena(Player player, ItemInstance item, NpcInstance fromNpc)
	{
		...
		for (Player member : membersInRange)
		{
			...
			member.getInventory().addAdena(count, "MonsterKill");
