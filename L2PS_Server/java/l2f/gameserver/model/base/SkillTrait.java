...
public enum SkillTrait
{
	...
	HOLD
	{
		...
		public final double calcVuln(Env env)
		{
			if(env.target.isPlayable() && env.character != null && env.character.isPlayable())
				return env.target.calcStat(Stats.REAL_ROOT_RESIST, env.character, env.skill);
	...
	SHOCK
	{
		...
		{
			if(env.target.isPlayable() && env.character != null && env.character.isPlayable())
				return env.target.calcStat(Stats.REAL_STUN_RESIST, env.character, env.skill);
